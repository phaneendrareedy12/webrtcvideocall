...
const client = new cassandra.Client({ contactPoints: ['192.168.209.67:9042'], localDataCenter: 'datacenter1', authProvider: authProvider})
client.connect(err => {
    if(!err) {
        console.log('connected to cassandra db ........')
    } else {
        console.log('error connnecting cassandra db....', err)
    }
});
...
app.post('/saveOfferAnswer', async(req,res) => {
    ...
    const query = 'INSERT INTO webrtc_data.webrtc_sdp (id, type, sdp, time_stamp) VALUES (?, ?, ?, toTimestamp(now()))'
    const params = [uuid(), req.body.sdp.type, req.body.sdp.sdp]
    client.execute(query, params, { prepare: true }, err => {
        if(!err) {
            console.log('sdp data inserted into webrtc_data.webrtc_sdp ........')
        } else {
            console.log('error inserted sdp data into webrtc_data.webrtc_sdp ........', err)
        }
      })
...
webRTCNamespace.on('connection', socket => {
    ...
    socket.on('sdp', data => {
        ...
        axios.post('http://localhost:8081/saveOfferAnswer', data)
        .then(response => {
            console.log("post called successfully")
        })
        .catch(error => {
            console.log("error occurred while calling post method which saves answer offer data into DB", error)
        }) 
    }) 
    ...
    socket.on('candidate', data => {
        ...
        // axios.post('http://localhost:8081/saveIceCandidates', data)
        // .then(response => {
        //     console.log("post called successfully")
        // })
        // .catch(error => {
